...
const formPageMap = {
	...
	email: ['email'],
	address: ['address_1', 'postcode', 'town'],
...
function getValues(keyArray) {
	...
	if (
		Object.keys(formProcessObject).length === 0 &&
		formProcessObject.constructor === Object
	) {
		...
		let getFromApiResult = getFromApi(
			getValueKeysNotInFormValuesGetMap(keyArray)
		)
	}
	//Process result
}
